...
def is_last_message_from_sender(chat_log, sender_name="Yash Baswar"):
...
while True:
    ...
    if is_last_message_from_sender(chat_history)
        completion = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a person named Akkya who speaks marathi as well as english. You are from India and you are a coder. You analyze chat history and respond like Akkya. Output should be the next chat response (text messages only)"},
                {
                    "role": "system", 
                    "content": "Do not start like this [21:02, 12/6/2024] Yash Baswar: "
                },
                {
                    "role": "user",
